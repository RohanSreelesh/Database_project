...
def Q_1(conn, execution_time):
    ...
    query = """SELECT s.player_name, AVG(s.statsbomb_xg) AS average_xg
FROM Shot s
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT s.player_name, COUNT(s.event_id) AS total_shots
FROM Shot s
JOIN matches m ON s.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'La Liga' AND c.season_name = '2020/2021'
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT s.player_name, COUNT(s.event_id) AS total_first_time_shots
FROM Shot s
JOIN matches m ON s.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'La Liga' AND s.first_time = true
GROUP BY s.player_name
HAVING COUNT(s.event_id) >= 1
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT p.team_name, COUNT(p.event_id) AS total_passes
FROM Pass p
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT pa.recipient_name AS player_name, COUNT(pa.recipient_name) AS total_passes_received
FROM Pass pa
JOIN matches m ON pa.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'Premier League' AND c.season_name = '2003/2004'
GROUP BY pa.recipient_name
HAVING COUNT(pa.recipient_name) >= 1
ORDER BY total_passes_received DESC;
 """
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT s.team_name, COUNT(s.event_id) AS total_shots
FROM Shot s
JOIN matches m ON s.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'Premier League' AND c.season_name = '2003/2004'
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT pa.player_name, COUNT(pa.event_id) AS total_through_balls
FROM Pass pa
JOIN matches m ON pa.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'La Liga' AND c.season_name = '2020/2021' AND pa.through_ball = true
...
def Q_8(conn, execution_time):
    ...
    query = """SELECT pa.team_name, COUNT(pa.event_id) AS total_through_balls
FROM Pass pa
WHERE pa.match_id IN (
    SELECT m.match_id
    FROM matches m
    JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
    WHERE c.competition_name = 'La Liga' AND c.season_name = '2020/2021'
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT d.player_name, COUNT(d.event_id) AS total_successful_dribbles
FROM Dribble d
JOIN matches m ON d.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
JOIN outcomes o ON d.outcome_id = o.outcome_id
WHERE c.competition_name = 'La Liga' 
  AND c.season_name IN ('2020/2021', '2019/2020', '2018/2019')
  AND o.outcome_name = 'Complete'
...
def Q_10(conn, execution_time):
    ...
    query = """SELECT dp.player_name, COUNT(dp.event_id) AS total_dribbled_past
FROM DribbledPast dp
JOIN matches m ON dp.match_id = m.match_id
JOIN competitions c ON m.competition_id = c.competition_id AND m.season_id = c.season_id
WHERE c.competition_name = 'La Liga' AND c.season_name = '2020/2021'
GROUP BY dp.player_name
HAVING COUNT(dp.event_id) >= 1
ORDER BY total_dribbled_past ASC; """
